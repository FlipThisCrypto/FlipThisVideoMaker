...
The deterministic CPU mock path, worker heartbeat lifecycle, active media cancellation, and
per-physical-GPU admission logic are implemented and exercised. External model adapters are only
prepared/configurable unless a document explicitly says a real backend was exercised. Two RTX 4070
devices are discoverable and their worker processes have been started independently, but no model
backend or CUDA generation workload has been exercised.
